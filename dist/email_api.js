...
export async function setAndGetMappedEmails() {
    let raw_emails = await getAllEmails();
    if (raw_emails === null) {
        console.log(`> emaileket nem tudtuk betölteni :( - api probléma`);
        exit();
    }
    ...
    let parsed_emails = raw_emails.results.map(mail => mail.address).map(email => {
        ...
        if (!id)
            console.log(`| email > ${spaced_email} > nincs :/`);
        else
            console.log(`| email > ${spaced_email} > ${id}`);
        return [email, id];
...
export function getMappedEmails() {
    return all_emails;
}
export async function refreshMappedEmails() {
    all_emails = await setAndGetMappedEmails();
}
// delta calculations
export function handleEmailDeltaCalculation(email, num) {
    if (num === 0) {
        delta_ins[email] = 0;
        return 0;
    }
...
export async function getEmailStats(email) {
    try {
