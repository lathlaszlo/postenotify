...
export function saveKeyValuePairIntoDatabase(key, value) {
    database[key] = value;
    console.log(`| set key '${key}' to '${value}'`);
    saveDatabase();
}
export function removeKeyValuePairFromDatabase(key) {
    if (database[key]) {
        delete database[key];
...
export function getValuePairFromKeyInDatabase(key) {
    if (database[key])
        return database[key];
    return null;
}
